...
public final class MediaPlaylistLinkHandlerFactory extends ListLinkHandlerFactory {

    private static final MediaPlaylistLinkHandlerFactory INSTANCE =
            new MediaPlaylistLinkHandlerFactory();

    private MediaPlaylistLinkHandlerFactory() {
    }

    public static MediaPlaylistLinkHandlerFactory getInstance() {
        return INSTANCE;
    }
...
// = "https://www.Media.com/playlist?list="
    }
     ...
     * like {@code https://Media.com/watch?v=videoId&list=playlistId}
    ...
    public ListLinkHandler fromUrl(final String url) throws ParsingException {
        try {
            final URL urlObj = Utils.stringToURL(url);
            final String listID = Utils.getQueryValue(urlObj, "list");
            if (listID != null && MediaParsingHelper.isMediaMixId(listID)) {
                String videoID = Utils.getQueryValue(urlObj, "v");
                if (videoID == null) {
                    videoID = MediaParsingHelper.extractVideoIdFromMixId(listID);
                }
