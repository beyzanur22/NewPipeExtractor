...
/**
 * The extractor of Media's base JavaScript player file.
 *
 * <p>
 ...
 *
 * <p>
 * It will try to get the player URL from Media's IFrame resource first, and from a Media embed
 * watch page as a fallback.
 * </p>
 */
final class MediaJavaScriptExtractor {
   ...
   private static final String BASE_JS_PLAYER_URL_FORMAT =
    ...
    private MediaJavaScriptExtractor() {
    }
    ...
    static String extractJavaScriptPlayerCode(@Nonnull final String videoId)
        ...
        String url;
        try {
            url = MediaJavaScriptExtractor.extractJavaScriptUrlWithIframeResource();
            final String playerJsUrl = MediaJavaScriptExtractor.cleanJavaScriptUrl(url);

            // Assert that the URL we extracted and built is valid
            new URL(playerJsUrl);

            return MediaJavaScriptExtractor.downloadJavaScriptCode(playerJsUrl);
        } catch (final Exception e) {
            url = MediaJavaScriptExtractor.extractJavaScriptUrlWithEmbedWatchPage(videoId);
            final String playerJsUrl = MediaJavaScriptExtractor.cleanJavaScriptUrl(url);
    ...
    private static String cleanJavaScriptUrl(@Nonnull final String javaScriptPlayerUrl) {
        if (javaScriptPlayerUrl.startsWith("//")) {
            ...
            // https://www.Media.com part has to be added manually if the URL is relative to
            // Media's domain
