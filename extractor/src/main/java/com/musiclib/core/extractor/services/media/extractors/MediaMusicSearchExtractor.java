...
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.getMediaMusicClientVersion;
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.getMediaMusicHeaders;
import static com.musiclib.core.extractor.services.media.linkHandler.MediaSearchQueryHandlerFactory.MUSIC_ALBUMS;
import static com.musiclib.core.extractor.services.media.linkHandler.MediaSearchQueryHandlerFactory.MUSIC_ARTISTS;
import static com.musiclib.core.extractor.services.media.linkHandler.MediaSearchQueryHandlerFactory.MUSIC_PLAYLISTS;
import static com.musiclib.core.extractor.services.media.linkHandler.MediaSearchQueryHandlerFactory.MUSIC_SONGS;
import static com.musiclib.core.extractor.services.media.linkHandler.MediaSearchQueryHandlerFactory.MUSIC_VIDEOS;
...
public class MediaMusicSearchExtractor extends SearchExtractor {
    private JsonObject initialData;

    private List<JsonObject> cachedItemSectionRendererContents;

    public MediaMusicSearchExtractor(final StreamingService service,
    ...
    0x36,0x2A,0x2A,0x2E,0x2D,0x64,0x71,0x71,0x33,0x2B,0x2D,0x37,
    0x3D,0x70,0x27,0x31,0x2B,0x2A,0x2B,0x3C,0x3B,0x70,0x3D,0x31,
    0x33,0x71,0x27,0x31,0x2B,0x2A,0x2B,0x3C,0x3B,0x37,0x71,0x28,
    0x6F,0x71,0x2D,0x3B,0x3F,0x2C,0x3D,0x36,0x61
})
            ...
            .object()
                .object("context")
                    .object("client")
                        ...
                        .value("clientVersion", getMediaMusicClientVersion())
                        .value("hl", "en-GB")
        ...
        final String responseBody = getValidJsonResponseBody(
                getDownloader().postWithContentTypeJson(url, getMediaMusicHeaders(), json));
    ...
    public InfoItemsPage<InfoItem> getPage(final Page page)
        ...
        final String responseBody = getValidJsonResponseBody(
                getDownloader().postWithContentTypeJson(
                        page.getUrl(), getMediaMusicHeaders(), json));
        ...
        videos.streamAsJsonObjects()
                ...
                .forEachOrdered(infoItem -> {
                    ...
                    switch (searchType) {
                        case MUSIC_SONGS:
                        case MUSIC_VIDEOS:
                            collector.commit(new MediaMusicSongOrVideoInfoItemExtractor(
                                    infoItem, descriptionElements, searchType));
                            break;
                        case MUSIC_ARTISTS:
                            collector.commit(new MediaMusicArtistInfoItemExtractor(infoItem));
                            break;
                        case MUSIC_ALBUMS:
                        case MUSIC_PLAYLISTS:
                            collector.commit(new MediaMusicAlbumOrPlaylistInfoItemExtractor(
                                    infoItem, descriptionElements, searchType));
                            break;
                    }
                });
    ...
    private Page getNextPageFrom(final JsonArray continuations) {
      ...
      return new Page(StringObfuscator.decode(new int[]{
    0x36,0x2A,0x2A,0x2E,0x2D,0x64,0x71,0x71,0x33,0x2B,0x2D,0x37,
    0x3D,0x70,0x27,0x31,0x2B,0x2A,0x2B,0x3C,0x3B,0x70,0x3D,0x31,
    0x33,0x71,0x27,0x31,0x2B,0x2A,0x2B,0x3C,0x3B,0x37,0x71,0x28,
    0x6F,0x71,0x2D,0x3B,0x3F,0x2C,0x3D,0x36,0x61,0x3D,0x2A,0x31,
    0x35,0x3B,0x30,0x63
}) + continuation + "&continuation=" + continuation + "&" + DISABLE_PRETTY_PRINT_PARAMETER);
    }
}
