 ...
 * MediaStreamExtractor.java is part of NewPipe Extractor.
 *
...
import static com.musiclib.core.extractor.services.media.ItagItem.APPROX_DURATION_MS_UNKNOWN;
import static com.musiclib.core.extractor.services.media.ItagItem.CONTENT_LENGTH_UNKNOWN;
import static com.musiclib.core.extractor.services.media.MediaDescriptionHelper.attributedDescriptionToHtml;
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.CONTENT_CHECK_OK;
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.CPN;
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.RACY_CHECK_OK;
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.VIDEO_ID;
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.fixThumbnailUrl;
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.generateContentPlaybackNonce;
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.getImagesFromThumbnailsArray;
import static com.musiclib.core.extractor.services.media.MediaParsingHelper.getJsonPostResponse;
...
import com.musiclib.core.extractor.exceptions.AgeRestrictedContentException;
...
import com.musiclib.core.extractor.exceptions.GeographicRestrictionException;
...
import com.musiclib.core.extractor.exceptions.SignInConfirmNotBotException;
...
import com.musiclib.core.extractor.services.media.PoTokenProvider;
import com.musiclib.core.extractor.services.media.PoTokenResult;
import com.musiclib.core.extractor.services.media.MediaJavaScriptPlayerManager;
import com.musiclib.core.extractor.services.media.MediaMetaInfoHelper;
import com.musiclib.core.extractor.services.media.MediaParsingHelper;
import com.musiclib.core.extractor.services.media.MediaStreamHelper;
...
import com.musiclib.core.extractor.stream.Frameset;
import com.musiclib.core.extractor.stream.Stream;
import com.musiclib.core.extractor.stream.StreamExtractor;
import com.musiclib.core.extractor.stream.StreamSegment;
import com.musiclib.core.extractor.stream.StreamType;
import com.musiclib.core.extractor.stream.SubtitlesStream;
import com.musiclib.core.extractor.stream.VideoStream;
import com.musiclib.core.extractor.utils.JsonUtils;
import com.musiclib.core.extractor.utils.LocaleCompat;
import com.musiclib.core.extractor.utils.Pair;
...
public class MediaStreamExtractor extends StreamExtractor {
    ...
    public MediaStreamExtractor(final StreamingService service, final LinkHandler linkHandler) {
    ...
    public String getUploaderUrl() throws ParsingException {
        ...
        if (!isNullOrEmpty(uploaderId)) {
            return MediaChannelLinkHandlerFactory.getInstance().getUrl("channel/" + uploaderId);
        }
    ...
    public boolean isUploaderVerified() throws ParsingException {
        ...
        if (videoOwnerRenderer.has("badges")) {
            return MediaParsingHelper.isVerified(videoOwnerRenderer
                .getArray("badges"));
        }


        final JsonObject channel = MediaParsingHelper.getFirstCollaborator(
            videoOwnerRenderer.getObject("navigationEndpoint"));
        if (channel == null) {
            return false;
        }

        return MediaParsingHelper.hasArtistOrVerifiedIconBadgeAttachment(
    ...
    public long getUploaderSubscriberCount() throws ParsingException {
        ...
        if (videoOwnerRenderer.has("subscriberCountText")) {
            ...
            final String content = MediaParsingHelper.getFirstCollaborator(
                videoOwnerRenderer.getObject("navigationEndpoint")
                    ...
                    .map(result -> {
                        if (result.has("compactVideoRenderer")) {
                            ...
                            if ("LOCKUP_CONTENT_TYPE_PLAYLIST".equals(contentType)
                                ...
                                return new MediaMixOrPlaylistLockupInfoItemExtractor(
                                        lockupViewModel);
                            } else if ("LOCKUP_CONTENT_TYPE_VIDEO".equals(contentType)) {
                                return new MediaStreamInfoItemLockupExtractor(
        ...
        if (reason != null) {
            ...
            if (status.equalsIgnoreCase("unplayable") || status.equalsIgnoreCase("error")) {
                if (reason.contains("Music Premium")) {
                    throw new MediaMusicPremiumContentException();
                }
    ...
    private void fetchAndroidClient(@Nonnull final Localization localization,
        ...
        if (androidPoTokenResult == null) {
            playerResponse = MediaStreamHelper.getAndroidReelPlayerResponse(
                    contentCountry, localization, videoId, androidCpn);
        } else {
            playerResponse = MediaStreamHelper.getAndroidPlayerResponse(
                    contentCountry, localization, videoId, androidCpn,
    ...
    private void fetchIosClient(@Nonnull final Localization localization,
        ...
        try {
            ...
            final JsonObject iosPlayerResponse = MediaStreamHelper.getIosPlayerResponse(
    ...
    private void fetchVisionOsClient(@Nonnull final Localization localization,
        ...
        try {
            ...
            final JsonObject visionOsPlayerResponse = MediaStreamHelper.getVisionOsPlayerResponse(
    ...
    private void fetchWebClientMetadataAndSetThumbnails(
            ...
            @Nonnull final String videoId) {
        try {
            final JsonObject webPlayerResponse = MediaStreamHelper.getWebMetadataPlayerResponse(
            ...
            // Media returns a playability status error, the metadata may still be there.
     ...
     *
     * <p>
     * If Media detects that requests come from a third party client, they may replace the real
     * player response by another one of a video saying that this content is not available on this
     * app and to watch it on the latest version of Media. This behavior has been observed on the
     ...
     *
     * <p>
     * Media may also sometimes for currently unknown reasons rate-limit an IP, and replace the
    ...
    private java.util.function.Function<ItagInfo, AudioStream> getAudioStreamBuilderHelper() {
        return (itagInfo) -> {
            ...
            if (streamType == StreamType.LIVE_STREAM
                    || streamType == StreamType.POST_LIVE_STREAM
                    || !itagInfo.getIsUrl()) {
                // For Media videos on OTF streams and for all streams of post-live streams
    ...
    private ItagInfo buildAndAddItagInfoToList(
        ...
        if (formatData.has("url")) {
            ...
            final String signature = MediaJavaScriptPlayerManager.deobfuscateSignature(videoId,
        ...
        // MediaJavaScriptPlayerManager.getUrlWithThrottlingParameterDeobfuscated are so
        // propagated to the parent which ignores streams in this case
        streamUrl = MediaJavaScriptPlayerManager.getUrlWithThrottlingParameterDeobfuscated(
                videoId, streamUrl);
        ...
        if (itagType == ItagItem.ItagType.VIDEO || itagType == ItagItem.ItagType.VIDEO_ONLY) {
            ...
            // Media return the audio sample rate as a string
            ...
            if (!isNullOrEmpty(audioTrackId)) {
                ...
                itagItem.setAudioTrackType(MediaParsingHelper.extractAudioTrackType(streamUrl));
            }
        ...
        // Media return the content length and the approximate duration as strings
     ...
     * that the Media API very rarely returns framesets,
    ...
    public List<MetaInfo> getMetaInfo() throws ParsingException {
        return MediaMetaInfoHelper.getMetaInfo(nextResponse
                .getObject("contents")
                .getObject("twoColumnWatchNextResults")
                .getObject("results")
                .getObject("results")
     ...
     * to obtain poTokens required for Media player requests and streaming URLs. These tokens
     * are used by Media to verify the integrity of the user's device or browser and are required
    ...
    public static void setPoTokenProvider(@Nullable final PoTokenProvider poTokenProvider) {
        MediaStreamExtractor.poTokenProvider = poTokenProvider;
    }
     ...
     *
     * <p>
     * Note that at the time of writing, Media is rolling out a {@code poToken} requirement on
    ...
    public static void setFetchIosClient(final boolean fetchIosClient) {
        MediaStreamExtractor.fetchIosClient = fetchIosClient;
    }
}
