...
import com.musiclib.core.extractor.utils.JavaScript;
...
/**
 * Manage the extraction and the usage of Media's player JavaScript needed data in the Media
 * service.
 *
 * <p>
 * Media restrict streaming their media in multiple ways by requiring their HTML5 clients to use
 ...
 * </p>
 */
public final class MediaJavaScriptPlayerManager {
    ...
    private MediaJavaScriptPlayerManager() {
    }
     ...
     *                passed, even it is not recommend in order to spoof better official Media
     *                clients)
    ...
    public static Integer getSignatureTimestamp(@Nonnull final String videoId)
        ...
        try {
            cachedSignatureTimestamp = Integer.valueOf(
                    MediaSignatureUtils.getSignatureTimestamp(cachedJavaScriptPlayerCode));
     ...
     *                            spoof better official Media clients)
    ...
    public static String deobfuscateSignature(@Nonnull final String videoId,
        ...
        if (cachedSignatureDeobfuscationFunction == null) {
            try {
                cachedSignatureDeobfuscationFunction = MediaSignatureUtils.getDeobfuscationCode(
        ...
        try {
            ...
            return Objects.requireNonNullElse(
                    JavaScript.run(cachedSignatureDeobfuscationFunction,
                            MediaSignatureUtils.DEOBFUSCATION_FUNCTION_NAME,
                            obfuscatedSignature), "");
        } catch (final Exception e) {
    ...
    public static String getUrlWithThrottlingParameterDeobfuscated(
        ...
        final String obfuscatedThrottlingParameter =
                MediaThrottlingParameterUtils.getThrottlingParameterFromStreamingUrl(
                        streamingUrl);
        ...
        if (cachedThrottlingDeobfuscationFunction == null) {
            try {
                cachedThrottlingDeobfuscationFunctionName =
                        MediaThrottlingParameterUtils.getDeobfuscationFunctionName(
                                cachedJavaScriptPlayerCode);

                cachedThrottlingDeobfuscationFunction =
                        MediaThrottlingParameterUtils.getDeobfuscationFunction(
                                cachedJavaScriptPlayerCode,
                                cachedThrottlingDeobfuscationFunctionName);
            } catch (final ParsingException e) {
                // Store the exception for future calls of this method, in order to improve
    ...
    private static void extractJavaScriptCodeIfNeeded(@Nonnull final String videoId)
        ...
        if (cachedJavaScriptPlayerCode == null) {
            cachedJavaScriptPlayerCode = MediaJavaScriptExtractor.extractJavaScriptPlayerCode(
                    videoId);
        }
    }
}
