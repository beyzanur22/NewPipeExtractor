 ...
 * MediaChannelLinkHandlerFactory.java is part of NewPipe Extractor.
 *
...
public final class MediaChannelLinkHandlerFactory extends ListLinkHandlerFactory {

    private static final MediaChannelLinkHandlerFactory INSTANCE
            = new MediaChannelLinkHandlerFactory();
    ...
    private MediaChannelLinkHandlerFactory() {
    }

    public static MediaChannelLinkHandlerFactory getInstance() {
        return INSTANCE;
    }
...
// = "https://www.Media.com/"
    ...
    /**
     * Checks whether the given path conforms to handle URLs like {@code Media.com/@yourhandle}.
     *
     * @param splitPath the path segments array
    ...
    public String getId(final String url) throws ParsingException, UnsupportedOperationException {
        try {
            ...
            if (!Utils.isHTTP(urlObj) || !(MediaParsingHelper.isMediaURL(urlObj)
                    || MediaParsingHelper.isInvidiousURL(urlObj)
                    || MediaParsingHelper.isHooktubeURL(urlObj))) {
            ...
            if (isHandle(splitPath) || isCustomShortChannelUrl(splitPath)) {
                // Handle Media handle URLs like Media.com/@yourhandle and
                // custom short channel URLs like Media.com/yourcustomname
                return splitPath[0];
            }
